...
#### DPPH Assay (n=70)
| Model | R² | RMSE | MAE | vs MK-Ensemble |
|-------|-----|------|-----|-----------|
| **MK-Ensemble v5** | **0.588** | 0.230 | 0.176 | - |
...
**结论**: MK-Ensemble与Random Forest性能相当（无显著差异），但显著优于XGBoost和PLS

#### ABTS Assay (n=42)
| Model | R² | RMSE | MAE | vs MK-Ensemble |
|-------|-----|------|-----|-----------|
| **MK-Ensemble v5** | **0.867** | 0.195 | 0.147 | - |
...
**结论**: MK-Ensemble数值上优于所有基线，但与RF无统计显著差异

#### FRAP Assay (n=16)
| Model | R² | RMSE | MAE | vs MK-Ensemble |
|-------|-----|------|-----|-----------|
| **MK-Ensemble v5** | **0.789** | 0.137 | 0.116 | - |
...
**结论**: MK-Ensemble显著优于Random Forest，但注意样本量仅n=16
...
### 1. 性能一致性
- **ABTS**: MK-Ensemble表现最佳（R²=0.867）
- **FRAP**: MK-Ensemble显著优于RF（R²=0.789 vs 0.727）
- **DPPH**: MK-Ensemble与RF相当（R²=0.588 vs 0.623），差异不显著

### 2. 统计显著性总结
- MK-Ensemble在FRAP上显著优于RF (p=0.003)
- MK-Ensemble在DPPH上显著优于XGBoost (p=0.019) 和 PLS (p=0.018)
- MK-Ensemble在ABTS上显著优于XGBoost (p=0.014) 和 SVR (p<0.001)
...
2. **与RF比较**: DPPH上MK-Ensemble并未显著优于RF (p=0.596)
3. **声称过于绝对**: 应修改为"MK-Ensemble achieves comparable or superior performance"
...
### 1. 修改性能声称
**原句**:
> MK-Ensemble achieved stronger predictive performance than baseline methods

**修改建议**:
> MK-Ensemble achieved comparable or superior predictive performance compared to baseline methods, with particularly strong results on ABTS (R² = 0.867) and FRAP (R² = 0.789) assays. On DPPH, MK-Ensemble performed comparably to Random Forest (R² = 0.588 vs 0.623, p = 0.596).

### 2. 添加统计显著性说明
在Results部分添加:
> Statistical comparisons using Wilcoxon signed-rank tests showed that MK-Ensemble performed significantly better than XGBoost and PLS on DPPH (p < 0.05), and significantly better than Random Forest on FRAP (p = 0.003). Performance differences between MK-Ensemble and Random Forest on DPPH and ABTS were not statistically significant.
...
- `experiments/phase1_fix_performance/results/benchmark_v5_ensemble.csv` - MK-Ensemble v5性能
